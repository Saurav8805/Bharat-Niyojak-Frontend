...
import {
  Mail, Phone, Calendar, Shield, Building2,
...
export default function AdminProfilePage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (u.role === 'super_admin') {
        router.push('/admin/super/profile');
        return;
      }
      if (!u.role?.includes('admin')) { router.push('/login'); return; }
      fetchProfile(u.id, token);
      fetchStats(u.department, token);
  ...
  const fetchStats = async (department: string, token: string) => {
    try {
      const res = await fetch(`${API_URL}/issues/admin/department?department=${department}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      if (data.success) {
  ...
  const avatarColor = getAvatarColor(profile?.full_name || 'A');
  const dept = profile?.department || '';
  const deptLabel = DEPARTMENT_LABELS[dept] || dept;
  const deptColor = DEPT_COLORS[dept] || 'bg-blue-500';
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="max-w-4xl mx-auto px-3 sm:px-6 py-4 sm:py-5 space-y-4">
            ...
            <div className="bg-white rounded-xl shadow-xs border border-gray-200 overflow-hidden hover:border-primary-400 hover:ring-2 hover:ring-primary-50 transition-all duration-200">
              <div className="h-16 bg-gradient-to-r from-primary-600 to-indigo-700" />
              <div className="px-4 sm:px-5 pb-4 sm:pb-5">
                <div className="flex items-end justify-between -mt-8 mb-3">
                  ...
                  <div className="flex gap-1.5 mt-8">
                    {editing ? (
                      <>
                        <button onClick={() => setEditing(false)} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs text-gray-600 hover:bg-gray-100 border border-gray-200 transition-colors">
                          <X className="w-3.5 h-3.5" /> Cancel
                        </button>
                        <button onClick={handleSave} disabled={saving} className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs bg-primary-600 text-white hover:bg-primary-700 transition-colors disabled:opacity-60 font-semibold shadow-2xs">
                          <Save className="w-3.5 h-3.5" /> {saving ? 'Saving…' : 'Save'}
                        </button>
                      </>
                    ) : (
                      <button onClick={() => setEditing(true)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs bg-primary-600 text-white hover:bg-primary-700 transition-colors font-semibold shadow-2xs">
                ...
                <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                  <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full bg-primary-100 text-primary-700">Admin</span>
                  {dept && (
                    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full text-white ${deptColor}`}>
                      {deptLabel}
                    </span>
                  )}
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4">
              ...
              <div className="md:col-span-2 bg-white rounded-xl shadow-xs border border-gray-200 p-4 sm:p-5 space-y-3.5 hover:border-primary-400 hover:ring-2 hover:ring-primary-50 transition-all duration-200">
                <h3 className="text-sm font-bold text-gray-900 border-b border-gray-100 pb-2.5">Account Information</h3>

                <InfoRow icon={<Mail className="w-4 h-4 text-primary-600" />} bg="bg-primary-50" label="Email Address" value={profile?.email} />
                ...
                <InfoRow icon={<Building2 className="w-4 h-4 text-amber-600" />} bg="bg-amber-50" label="Department" value={deptLabel || 'Not assigned'} />
                <InfoRow icon={<Calendar className="w-4 h-4 text-indigo-600" />} bg="bg-indigo-50" label="Member Since" value={joinDate} />
                <div className="flex items-center gap-3">
              ...
              <div className="bg-white rounded-xl shadow-xs border border-gray-200 p-4 sm:p-5 hover:border-primary-400 hover:ring-2 hover:ring-primary-50 transition-all duration-200">
                <h3 className="text-sm font-bold text-gray-900 border-b border-gray-100 pb-2.5 mb-3">Department Stats</h3>
                <div className="space-y-2">
                  {[
                    { label: 'Total', value: stats.total, color: 'bg-primary-600' },
                    { label: 'Pending', value: stats.pending, color: 'bg-amber-500' },
                    { label: 'In Progress', value: stats.in_progress, color: 'bg-purple-600' },
                    { label: 'Resolved', value: stats.resolved, color: 'bg-emerald-600' },
...
function InfoRow({ icon, bg, label, value }: { icon: React.ReactNode; bg: string; label: string; value?: string | null }) {
  return (
    <div className="flex items-center gap-3">
      <div className={`w-8 h-8 rounded-lg ${bg} flex items-center justify-center shrink-0`}>{icon}</div>
      <div>
        <p className="text-[10px] text-gray-500 mb-0.5">{label}</p>
        <p className="text-xs font-semibold text-gray-900">{value || <span className="text-gray-400 font-normal">—</span>}</p>
      </div>
    </div>
  );
}
