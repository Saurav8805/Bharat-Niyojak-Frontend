...
import { ArrowRight, Check, Zap, Shield, TrendingUp, Users, MapPin, Clock } from 'lucide-react';
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              <div key={index} className="group relative">
                <div className="bg-white p-8 rounded-2xl shadow-lg border-2 border-gray-100 hover:border-primary-200 transition-all hover:shadow-2xl transform hover:-translate-y-2">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              ...
              <div key={index} className="bg-white p-8 rounded-2xl shadow-lg hover:shadow-2xl transition-all border border-gray-100 hover:border-primary-200 group">
      ...
      <section className="py-20 bg-white/85 dark:bg-gray-800/85 backdrop-blur-sm relative transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {[
              { icon: '🕳️', name: 'Potholes', color: 'hover:border-orange-400 hover:bg-orange-50' },
              { icon: '🛣️', name: 'Damaged Roads', color: 'hover:border-gray-400 hover:bg-gray-50' },
              { icon: '🗑️', name: 'Garbage', color: 'hover:border-green-400 hover:bg-green-50' },
              { icon: '💡', name: 'Streetlights', color: 'hover:border-yellow-400 hover:bg-yellow-50' },
              { icon: '💧', name: 'Water Leakage', color: 'hover:border-blue-400 hover:bg-blue-50' },
              { icon: '🌳', name: 'Fallen Trees', color: 'hover:border-emerald-400 hover:bg-emerald-50' },
              { icon: '🚰', name: 'Drainage', color: 'hover:border-cyan-400 hover:bg-cyan-50' },
              { icon: '⚠️', name: 'Open Manholes', color: 'hover:border-red-400 hover:bg-red-50' },
              { icon: '🚦', name: 'Traffic Issues', color: 'hover:border-amber-400 hover:bg-amber-50' },
              { icon: '🏗️', name: 'Infrastructure', color: 'hover:border-slate-400 hover:bg-slate-50' },
            ].map((item, index) => (
              <div 
                key={index}
                className={`bg-white p-6 rounded-xl shadow-md border-2 border-gray-200 text-center transition-all cursor-pointer transform hover:scale-105 hover:shadow-xl ${item.color}`}
              >
